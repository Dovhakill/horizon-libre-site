...
def generate_tweet(title, url, category):
    ...
    prompt = f"""
    Agis comme un community manager expert pour "L'Horizon Libre".
    Rédige un tweet percutant pour l'article suivant.
    Le tweet doit avoir une accroche intrigante (1-2 phrases), 3 à 5 hashtags pertinents, et le lien vers l'article.
    Ne pas dépasser 280 caractères.

    Titre : "{title}"
    Catégorie : "{category}"
    Lien : {url}

    Rédige uniquement le texte du tweet.
    """
...
def post_tweet(text, image_path=None):
    ...
    try:
        ...
        client.create_tweet(text=text, media_ids=media_ids if media_ids else None)
...
def main():
    ...
    if os.path.exists(TEMP_IMAGE_PATH):
        ...
        print(f"Image temporaire '{TEMP_IMAGE_PATH}' supprimée.")
