...
def mark(key: str):
    ...
    requests.post(
        ...
        json={"key": full_key, "meta": {"tweeted_at": "some_timestamp"}}, 
        timeout=20
    ).raise_for_status()

# --- Fonctions de Contenu et de Tweet ---
...
def generate_tweet(title, url, category):
    ...
    prompt = f"""Rédige un tweet percutant pour l'article : Titre: "{title}", Catégorie: "{category}", Lien: {url}. Inclus 3-5 hashtags pertinents. Ne dépasse pas 280 caractères."""
...
def main():
    ...
    key = topic_key(str(latest_article_path))
